package com.dl.mediapicker.util;
        ...
        public void run() {
            ...
            while (data.moveToNext()) {
                ...
                if(mimeType.contains("video")){//视频
                    ...
                    Cursor thumbCursor = contentResolver.query(MediaStore.Video.Thumbnails.EXTERNAL_CONTENT_URI, null, selection, selectionArgs, null);
                    if(thumbCursor!=null&&thumbCursor.moveToFirst()){
                        String thumbnailPath = thumbCursor.getString(thumbCursor.getColumnIndexOrThrow(MediaStore.Video.Thumbnails.DATA));
                        photo = new Photo(imageId, path, mimeType, width, height, size,thumbnailPath,true);
                    ...
                    photo = new Photo(imageId, path, mimeType, width, height, size,"",false);
                }
                if(photo!=null){
                    photo.setAdddate(addDate);
                }
                PhotoDirectory photoDirectory = null;
                for (PhotoDirectory dir : directories) {//首次drectories为空
                    if (dir.getId() == bucketId) {
                        photoDirectory = dir;
                        break;
                    }
                }
                if (photoDirectory == null&&!mimeType.contains("video")) {//生成新的路径，并填充到路径列表中
                ...
                if(photoDirectory != null&&!mimeType.contains("video")){
